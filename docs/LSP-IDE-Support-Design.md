...
| Q22 | A feature URI owned by N projects (linked/shared `.feature`) was routed and cached as if it had exactly one owner — nondeterministic `FirstOrDefault` display routing (Anomaly B) plus a URI-only match cache that bled F14 usages across projects. How should display and the match cache key shared files? | Chris | **Resolved (implemented)** — deterministic primary-owner (home-project) rule for single-URI display; binding-match cache re-keyed per `(uri, project)` for owner-set accounting. See [Q22 scope & as-built](Q22-uri-project-keying-scope.md). |
...
5. **Feature-aware operations split by layer — display is single-owner, accounting is owner-set.** LSP permits one diagnostic/semantic-token set per URI, so the *display* of an open shared feature (tokens + unmatched-step diagnostics) runs against a single **deterministic primary owner**: the home project (longest `ProjectFolder` prefix of the file), with an ordinal `ProjectFullName` tiebreak for files external to every owner's folder (see [Q22](Q22-uri-project-keying-scope.md)). The owner-*set* semantics survive only in the binding-match cache, which is keyed per `(uri, project)`: [F14](#f14--find-step-definition-usages) unions a binding's usages across all including projects and [F15](#f15--find-unused-step-definitions) intersects (unused only if unused in *every* including project), each project's match set computed against its own registry by the index-driven closed-file scan.
